...
export default {
  ...
  methods: {
    ...
    doFetchPrices() {
      ...
      axios.get('http://54.65.204.164.164:8082/drinks/' + drink_id + '/prices')
      .then(response => {
    ...
    doAddFavoriteShop(item){
      ...
      if (this.$store.state.login.id != 0){
        ...
        axios.post('http://54.65.204.164.164:8082/favorite_shop/add', params)
        .catch(error => {
          // handle error
          console.log(error)
        })
    ...
    doDeleteFavoriteShop(item){
      ...
      if (this.$store.state.login.id != 0){
        ...
        params.append('shop_id', item.ID)
        params.append('user_id', this.$store.state.login.id)
        axios.post('http://54.65.204.164.164:8082/favorite_shop/delete', params)
        .catch(error => {
          // handle error
          console.log(error)
        })
