...
export default {
  ...
  actions: {
    ...
    doFetchFavoriteDrinks(context, user_id) {
      if (user_id != 0){
        return axios.get('https://kakaku-real-store.tk:8082/user/'+ user_id + '/favorite_drink')
        .then(response => {
    ...
    doFetchFavoriteShops(context, user_id) {
      if (user_id != 0){
        return axios.get('https://kakaku-real-store.tk:8082/user/'+ user_id + '/favorite_shop')
        .then(response => {
